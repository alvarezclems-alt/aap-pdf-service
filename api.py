...
def _label_variants(label):
    ...
    variants = {
        ...
        _normalize_text(base).replace(" :", ":"),
    }
    if base.endswith(":"):
        variants.add(base[:-1].strip())
    if "é" in base or "è" in base:
        variants.add(base.replace("é", "e").replace("è", "e"))
    return [v for v in variants if v]

def _search_label_rects(page, label):
    found = []
    seen = set()
    for cand in _label_variants(label):
    ...
    try:
        ...
        if ext == ".pdf":
            ...
            try:
                ...
                norm_pdf_text = _normalize_text(pdf_text)
                sfpc_form = (
                    "formulaire de remboursement" in norm_pdf_text
                    and "congres sfpc" in norm_pdf_text
                )
                if sfpc_form:
                ...
                for page_num in range(doc_pdf.page_count):
                    ...
                    for label, config in ai_mapping.items():
                        if not isinstance(config, dict):
                            continue
                        rects = _search_label_rects(page, str(label))
                    ...
                    for label, config in ai_mapping.items():
                        ...
                        if atype == "column":
                            ...
                            col_x = r.x1 + 80
                            if col_header:
                                h_rects = _search_label_rects(page, str(col_header))
