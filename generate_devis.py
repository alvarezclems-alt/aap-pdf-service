...
def on_page(canvas, doc):
    canvas.saveState()
    canvas.setFont(FONT if FONT != "Helvetica" else "Helvetica", 8)
    canvas.setFillColor(colors.Color(0.6, 0.6, 0.6))
    canvas.drawCentredString(W / 2, 12 * mm, f"Page {canvas.getPageNumber()}")
    canvas.restoreState()
...
def generate_devis_pdf(data: dict) -> bytes:
    ...
    doc = SimpleDocTemplate(
        ...
        title=f"Devis {sanitize(data.get('devis', {}).get('numero', ''))}",
    )
    ...
    for line in sanitize(cl.get("adresse", "")).split("\n"):
        if line.strip():
    ...
    right_lines = [
        ...
        SP(2),
        P(f"Facture n° {sanitize(dv.get('numero', ''))}", sBold),
    ]
    ...
    # ── VILLE ET DATE ─────────────────────────────────────────────────────────
    ville = sanitize(dv.get("ville_emission", "Lille"))
    ...
    for ligne in lg:
        ...
        qte = sanitize(ligne.get("quantite", ""))
        ...
        if isinstance(tot_ligne, (int, float)) and tot_ligne != 0:
            ...
            tot_str = sanitize(tot_ligne)

        table_data.append([
            P(desc, sNormal),
            P(qte, sCenter),
            P(sanitize(pu), sCenter),
            P(tot_str, sRight),
        ])

    mention_tva = sanitize(tot.get("mention_tva", "Montant non assujetti à la TVA"))
    ...
    if exonere:
        ...
        tva_taux = sanitize(tot.get("tva_taux", 20))
        table_data.append([
            P("Montant HT", sSmall),
            P("", sSmall),
            P("", sSmall),
    ...
    if em.get("nom"):
        contact_parts.append(sanitize(em["nom"]))
    if em.get("tel"):
        contact_parts.append(f"Tél : {sanitize(em['tel'])}")
    if em.get("email"):
        contact_parts.append(f"Email : {sanitize(em['email'])}")

    story.append(P("   ".join(contact_parts), sSmall))

    if em.get("siret"):
        story.append(P(f"SIRET : {sanitize(em['siret'])}", sSmall))
