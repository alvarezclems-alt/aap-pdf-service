...
def S(name, size=9, bold=False, italic=False,
    ...
    if FONT == "Helvetica":
        fn = ("Helvetica-Bold" if bold and not italic else
...
def check(val, label, s_on=None, s_off=None):
    ...
    return P(f"{mark}  {sanitize(label)}", style)


def on_page(canvas, doc):
    canvas.saveState()
    canvas.setFont(FONT if FONT != "Helvetica" else "Helvetica", 7)
...
def generate_ordre_mission_pdf(data: dict) -> bytes:
    ...
    if type_depl in ("partiel", "sans_frais") and co_financeur:
        story.append(SP(1))
        story.append(P(f"Organisme co-financeur / nature des frais : {sanitize(co_financeur)}", sN))
    story.append(SP(3))

    # ── MISSIONNAIRE ──────────────────────────────────────────────────────────
    composante = mis.get("composante", "Laboratoire GERiiCO (ULR 4073)")
    miss_data = [[
        P(f"NOM : {sanitize(mis.get('nom', '')).upper()}", sB),
        P(f"Prénom : {sanitize(mis.get('prenom', ''))}", sB),
        P(f"Composante / Service : {sanitize(composante)}", sB),
    ]]
    ...
    dates_data = [[
        ...
        P(f"Heure de départ : {sanitize(msn.get('heure_debut', ''))}", sN),
        P(f"Date de retour : {fmt_date(msn.get('date_fin'))}", sN),
        P(f"Heure de retour : {sanitize(msn.get('heure_fin', ''))}", sN),
    ], [
        P(f"Lieu : {sanitize(msn.get('destination', ''))}", sN),
        P(""),
        P(f"Pays : {sanitize(msn.get('pays', 'France'))}", sN),
        P(""),
    ]]
    ...
    story.append(SP(2))
    story.append(P(f"OBJET : {sanitize(msn.get('objet', ''))}", sB))
    story.append(SP(3))
    ...
    if etranger:
        ...
        etr_rows = [
            ...
            [P("Avant tout déplacement à risque sanitaire, se rapprocher du Médecin de prévention 2 mois avant le départ.", sSub)],
            [P(f"Avis du Médecin : {sanitize(etr.get('avis_medecin', 'FAVORABLE AU DEPART  /  DEFAVORABLE AU DEPART'))}", sOr)],
        ]
        etr_t = Table([[P("Déplacement à l'étranger :", sB)]] + etr_rows, colWidths=[CW])
    ...
    tr_data = [
        ...
        [check(trp.get("transports_commun"), "Transports en commun"),
         ...
         P("", sC), P("", sC)],
        [P(f"Autres : {sanitize(trp.get('autre', '') or '_______________')}", sN),
         P(""), P(""), P("")],
    ]
    ...
    if trp.get("vehicule_personnel"):
        motif = sanitize(trp.get("motif_vehicule", "")).lower()
        ...
        vp_final = Table([
            ...
            [
                Table([[
                    ...
                    check("temps" in motif or "économie" in motif or "economie" in motif, "Gain de temps"),
    ...
    frais_data = [
        ...
        [P(f"Nb de repas à rembourser : {sanitize(nb_r)}     Nb de nuitées à rembourser : {sanitize(nb_n)}", sN)],
        [P(f"Autres dépenses (nature et montant) : {sanitize(autres)}", sN)],
        [P(f"Demande d'avance : {'OUI' if avance else '__________'}     Montant : {sanitize(montant) if avance else '__________'}", sB if avance else sN)],
    ]
    ...
    story.append(P(
        f"Centre de coût : {sanitize(cc)}     "
        f"Élément d'OTP : {sanitize(otp)}     "
        f"Domaine fonctionnel : {sanitize(dom)}", sN
    ))
    story.append(SP(4))
